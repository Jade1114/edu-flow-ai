...
def base_sample(item: dict[str, Any], sample_id: str, label: float, weight: float, reason: str) -> dict[str, Any]:
    ...
    capacity_ratio = round(room_capacity / total_students, 4) if total_students else 0.0
    ...
    return {
        ...
        "is_morning": 1 if period_index <= 2 else 0,
        "is_afternoon": 1 if 3 <= period_index <= 5 else 0,
        "is_evening": 1 if period_index >= 6 else 0,
        "is_early_period": 1 if period_index <= 2 else 0,
        ...
        "is_room_type_match": 1,
        "has_teacher_conflict": 0,
        "has_class_conflict": 0,
        "has_room_conflict": 0,
        "has_hard_conflict": 1 if has_conflict else 0,
