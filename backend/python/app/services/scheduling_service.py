...
from app.ml.placement_direct import DirectPlacementModel
from app.scheduling.templates import (
...
DEFAULT_TOP_K = int(os.getenv("ML_TOP_K", "50"))
MAX_TOP_K = int(os.getenv("ML_MAX_TOP_K", "200"))
MAX_PLAN_COUNT = int(os.getenv("ML_MAX_PLAN_COUNT", "500"))
DEFAULT_PLAN_COUNT = int(os.getenv("ML_PLAN_COUNT", "120"))
DEFAULT_CP_PLAN_COUNT = int(os.getenv("ML_CP_PLAN_COUNT", "40"))
MAX_SOLVER_TIME_LIMIT_SECONDS = float(os.getenv("ML_MAX_SOLVER_TIME", "3600.0"))
...
OUTPUT_ROOT = Path(__file__).resolve().parents[3] / "data" / "generated" / "v3"
...
def run_v3_pipeline(
    ...
    *,
    ...
    def emit(stage: str, message: str, progress: int, **extra: Any) -> None:
        ...
        log_line = f"[V3][{stage}] {message}"
        if progress is not None:
            log_line += f" ({progress}%)"
        service.info("%s", log_line)
    ...
    service.info(f"  Tasks: {len(teaching_tasks)}, Courses: {len(courses)}, "
    ...
    service.info("[V3] Step 2: Placement Model inference...")
    ...
    service.info(f"  {task_count} tasks → {candidates_path}")

    # ── Step 3: Template generation ─────────────────────────────────
    emit("PREFLIGHT", "正在生成 task plans，用于 CP-SAT 全局选择...", 32, raw_plan_count=resolved_plan_count)
    service.info("[V3] Step 3: Template generation...")
    ...
    service.info(f"  {task_plans_count} tasks → {task_plans_path}")
    ...
    service.info("[V3] Step 4: CP-SAT global plan selection...")
    ...
    # 修正 classroom_id：scheme 里的 ID 是旧模型的，用 classroom_name 映射到当前 DB 的 ID
    _remap_classroom_ids(schemes_path, classrooms)

    conflicts_after = _audit_schemes_jsonl(schemes_path)
    service.info(f"  Schemes: {cp_sat_summary['scheme_count']}/{resolved_scheme_count}; "
    ...
    service.info(f"[V3] Done in {runtime_s}s. Conflicts: {conflicts_after}")
    service.info(f"  → {schemes_path}")
    return summary
...
def run_v3_auto_pipeline(
    ...
    *,
    ...
    if _auto_stage_enabled("QUALITY_OPTIMIZATION", resolved_max_auto_stage):
        ...
        if quality_result is not None:
            ...
            stages.append({"stage": "QUALITY_OPTIMIZATION", "status": "SKIPPED", "reason": "QUALITY_FAILED"})
            emit("QUALITY_OPTIMIZATION", "质量优化未产出方案", 38)
    ...
    if _auto_stage_enabled("DIVERSITY_SEARCH", resolved_max_auto_stage) and quality_result is not None:
        ...
        stages.append({"stage": "DIVERSITY_SEARCH", "status": "SKIPPED", "reason": "QUALITY_FAILED"})
...
def _auto_first_feasible_time_limit(configured_time_limit: float) -> float:
    return max(30.0, min(float(configured_time_limit), 600.0))
...
def _widen_first_feasible_candidates(
    *,
    ...
    if task_count < LARGE_STRESS_TASK_THRESHOLD:
        return placement_top_k, raw_plan_count, cp_plan_count
    if generation_mode in FEASIBILITY_MODES:
        return (
    ...
    # 非 FEASIBILITY 模式（如 QUALITY）对大任务也扩宽候选集
    return (
        max(placement_top_k, FIRST_FEASIBLE_MIN_TOP_K),
        max(raw_plan_count, FIRST_FEASIBLE_MIN_PLAN_COUNT // 2),
        max(cp_plan_count, FIRST_FEASIBLE_MIN_CP_PLAN_COUNT // 2),
    )
...
def _remap_classroom_ids(schemes_path: Path, classrooms: dict[str, dict]) -> None:
    """修正 schemes.jsonl 中的 classroom_id。

    流程：
      1. 用 classroom_name → DB classroom.id 映射替换旧模型 ID
      2. 对于 DB 中不存在的教室，从同类型可用池中选一个替补
    """
    ...
    ji_fang_pool: list[int] = []
    for name, record in classrooms.items():
        cid = int(record["id"])
        if name.startswith("9"):
            ji_fang_pool.append(cid)
        else:
    ...
    substituted = 0
    import itertools
    normal_iter = itertools.cycle(normal_pool) if normal_pool else itertools.cycle([0])
    ji_fang_iter = itertools.cycle(ji_fang_pool) if ji_fang_pool else itertools.cycle([0])

    for line in schemes_path.read_text(encoding="utf-8").splitlines():
        if not line.strip():
            continue
        scheme = json.loads(line)
        for item in scheme.get("items") or []:
            name = str(item.get("classroom_name") or "").strip()
            if name and name in classrooms:
                new_id = int(classrooms[name]["id"])
                if int(item.get("classroom_id") or 0) != new_id:
                    item["classroom_id"] = new_id
                    fixed += 1
            elif name:
                # 教室不在 DB 中，从同类型池中替补一个
                pool = ji_fang_iter if name.startswith("9") else normal_iter
                sub_id = next(pool)
                item["classroom_id"] = sub_id
                substituted += 1
        lines.append(json.dumps(scheme, ensure_ascii=False, default=str))

    if fixed or substituted:
        schemes_path.write_text("\n".join(lines), encoding="utf-8")
        service.info(f"  [教室映射] 已修正 {fixed} 条 classroom_id, 替补 {substituted} 条缺失教室")
