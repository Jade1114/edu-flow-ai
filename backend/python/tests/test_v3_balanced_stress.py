...
from python.scripts.v3_balanced_stress import StressProfile, _balanced_rows, _distribution
...
class V3BalancedStressTest(unittest.TestCase):
    ...
    def test_auto_defaults_stop_after_quality(self):
        try:
            from python.scheduling_v3.pipeline import _resolve_max_auto_stage, _widen_first_feasible_candidates
    ...
    def test_infeasible_with_clean_preflight_reports_candidate_coverage(self):
        try:
            from python.scheduling_v3.cp_sat_selector import _candidate_coverage_diagnosis
        except SystemExit as exc:
            self.skipTest(str(exc))
