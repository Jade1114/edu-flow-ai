...
│   ├── 12-数据闭环与画像演进设计.md
│   └── 13-评分体系与约束分层设计.md
│   ├── 14-双通道排课架构设计.md（历史参考）
│   ├── 15-基于候选空间压缩与学习评分引导的智能排课架构.md（当前）
├── implementation/
│   ├── 01-GA排课生成链路实现说明.md
...
| architecture/09-GA编码与适应度函数设计.md | GA染色体/基因编码、硬软约束、适应度双管道（penalty_count + quality_score）、Deb 2000 可行性优先选择、遗传操作和答辩口径 |
| architecture/13-评分体系与约束分层设计.md | 5层评分架构（L0-L5）、penalty/quality分流、config参数设计、LLM约束挂载、与Deb 2000的理论对齐 |
...
选择：可行性优先 (Deb 2000) — penalty_count(可行性) + quality_score(L2+L3+L5) 双管道
```
