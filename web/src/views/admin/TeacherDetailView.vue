...
const parsedPreference = ref<Record<string, unknown> | null>(null)
...
async function loadProfile() {
    ...
    try {
        const data = await request.get(`/api/teachers/${teacherId.value}/profile`)
        if (data) {
            profile.value = {
            ...
            parseInterpretation.value = String(parsedPreference.value?.summary || '')
        } else {
            profile.value = {
                availabilityMatrix: defaultMatrix(),
                profileNote: '',
...
function normalizeMatrix(raw: unknown) {
    ...
    return Array.from({ length: 5 }, (_, periodIndex) =>
        Array.from({ length: 7 }, (_, weekdayIndex) => {
            const value = (matrix as any)?.[periodIndex]?.[weekdayIndex]
            return [-1, 0, 1].includes(value) ? value : 0
        }),
    )
}

function parseMatrix(json?: string) {
...
function parsePreferenceJson(json?: string) {
    if (!json) return null
    try {
...
function nextAvailabilityValue(value: number) {
    if (value === 0) return 1
    if (value === 1) return -1
    return 0
}

function toggleCell(periodIndex: number, weekdayIndex: number) {
...
async function parseProfile() {
    ...
    parsing.value = true
    try {
        const data = await request.post(`/api/teachers/${teacherId.value}/profile/parse`, {
...
async function saveProfile() {
    ...
    saving.value = true
    try {
        await request.put(`/api/teachers/${teacherId.value}/profile`, {
            availabilityMatrixJson: JSON.stringify(normalizeMatrix(profile.value.availabilityMatrix)),
            profileNote: profile.value.profileNote,
            profilePreferenceJson: profile.value.profilePreferenceJson,
        })
        ElMessage.success('教师画像已保存，返回画像页刷新后可看到合并结果')
        await loadProfile()
    } finally {
        saving.value = false
    }
...
<template>
    <div>
        <el-card v-loading="loading">
            <template #header>
                <div class="detail-header">
                    <span>教师详情</span>
                    <el-button size="small" @click="backToTeacherList">返回</el-button>
                </div>
            </template>
            ...
            <el-descriptions v-else-if="teacher" :column="2" border>
                ...
                <el-descriptions-item label="部门">{{ teacher.department || '-' }}</el-descriptions-item>
        ...
        <el-card id="profile" class="profile-card" v-loading="profileLoading">
            <template #header>
                <div class="detail-header">
                    <span>教师画像编辑</span>
                    <el-tag size="small" type="info">保存后将参与 V3 画像合并</el-tag>
                </div>
            </template>
            ...
            <el-form :model="profile" label-width="120px">
                ...
                <el-form-item>
                    <el-button
                        type="success"
                        :loading="saving"
                        :disabled="!!profile.profileNote?.trim() && !profile.profilePreferenceJson"
                        @click="saveProfile"
                    >
                        确认并保存画像
                    </el-button>
