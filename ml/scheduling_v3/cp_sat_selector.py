...
def select_cp_sat_global_plans_jsonl(
    ...
    *,
    ...
    if not schemes and (statuses[-1] if statuses else "UNKNOWN") == "UNKNOWN":
        fallback_scheme, fallback_summary = _atomic_session_fallback_scheme(1, tasks)
        if fallback_scheme and fallback_scheme["hard_conflicts"] == 0:
            statuses.append("FEASIBLE_ATOMIC_SESSION_FALLBACK")
