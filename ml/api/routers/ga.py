...
from ml.api import task_store
from ml.api.schemas import GenerateSchemeRequest, TaskStatusResponse

router = APIRouter(tags=["ga"])


def _run_pipeline_by_task(task_id: int, _ml_dir: Path) -> dict[str, Any]:
    """Blocking wrapper — lazily imports and runs GA pipeline by task_id."""
    generate_scheme_ga = importlib.import_module("ml.scripts.generate_scheme_ga")
    return generate_scheme_ga.run_ga_pipeline_by_task(task_id=task_id)
