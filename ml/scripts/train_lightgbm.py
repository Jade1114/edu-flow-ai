...
def main() -> None:
    ...
    result = run_training_pipeline(
        training_data_dir=str(args.data),
        output_model_path=str(args.model),
        output_schema_path=str(args.schema),
    )
    metrics = result["metrics"]
    validation = result["validation"]

    print(f"Trained LightGBM schedule scorer with {result['sample_count']} samples")
    print(f"Features: {result['feature_count']} total, {result['categorical_feature_count']} categorical")
    ...
    print(f"Feature schema saved -> {result['schema_path']}")
