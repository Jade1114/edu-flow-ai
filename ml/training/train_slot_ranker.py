"""Train the LightGBM slot ranker.

The trained model ranks (day, period) slots for a teaching task.
"""
...
from ml.scheduling_v2.slot_ranker import SLOT_RANK_FEATURES
from ml.training.build_slot_ranker_training_data import build as build_samples

DATA = Path(__file__).resolve().parents[2] / "data" / "real-dataset"
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "models" / "v2"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)

SAMPLES_PATH = DATA / "slot_ranker_samples.csv"
STATS_PATH = DATA / "slot_ranker_stats.json"
LABEL = "label"
...
def train() -> Path:
    ...
    print("加载 slot ranker 训练数据...")
    df = pd.read_csv(SAMPLES_PATH)
    missing_features = [feature for feature in SLOT_RANK_FEATURES if feature not in df.columns]
    if missing_features:
    ...
    x_train = train_df[SLOT_RANK_FEATURES]
    y_train = train_df[LABEL]
    x_test = test_df[SLOT_RANK_FEATURES]
    ...
    model_path = OUTPUT_DIR / "slot_ranker.txt"
    ...
    schema = {
        ...
        "model_type": "lightgbm_binary_slot_ranker",
        "auc": round(float(auc), 4),
        "accuracy": round(float(acc), 4),
        **{key: round(float(value), 4) for key, value in hit_metrics.items()},
        "training_samples": int(len(df)),
        "notes": "teaching_task + (day, period) -> slot rank score",
    }
    schema_path = OUTPUT_DIR / "slot_ranker_feature_schema.json"
    schema_path.write_text(json.dumps(schema, ensure_ascii=False, indent=2))
    print(f"Schema: {schema_path}")
    if STATS_PATH.exists():
        stats_output = OUTPUT_DIR / "slot_ranker_stats.json"
...
def _hit_at_k(test_df: pd.DataFrame, predictions, *, ks: tuple[int, ...]) -> dict[str, float]:
    scored = test_df[["course_code", "class_group", "label", "day", "period"]].copy()
