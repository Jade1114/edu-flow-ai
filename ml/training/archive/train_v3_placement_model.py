"""Train the V3 placement model.

Model contract:
  input identity = course_name + teacher_no + teacher_name + class_name
  ranked output  = day_of_week + period_index + classroom_name
"""

from __future__ import annotations

import json
from pathlib import Path

import lightgbm as lgb
import pandas as pd
from sklearn.metrics import accuracy_score, roc_auc_score
from sklearn.model_selection import GroupShuffleSplit

DATA_PATH = Path(__file__).resolve().parents[2] / "data" / "real-dataset" / "v3_placement_training_samples.csv"
OUTPUT_DIR = Path(__file__).resolve().parents[1] / "models" / "v3"
OUTPUT_DIR.mkdir(parents=True, exist_ok=True)
LABEL = "label"

FEATURES = [
    "course_name_code",
    "course_code_code",
    "teacher_no_code",
    "teacher_name_code",
    "class_name_code",
    "class_major_code",
    "class_department_code",
    "class_grade",
    "class_no",
    "student_count",
    "total_hours",
    "course_type_code",
    "required_room_type_code",
    "day_of_week",
    "period_index",
    "classroom_name_code",
    "classroom_type_code",
    "classroom_capacity",
    "capacity_margin",
    "capacity_ratio",
    "is_room_type_match",
]


def train(data_path: Path = DATA_PATH) -> Path:
    if not data_path.exists():
        raise FileNotFoundError(f"Training samples not found: {data_path}")

    print(f"加载 V3 placement 样本: {data_path}")
    raw_df = pd.read_csv(data_path)
    raw_df.columns = [str(column).strip() for column in raw_df.columns]
    for column in raw_df.select_dtypes(include="object").columns:
        raw_df[column] = raw_df[column].astype(str).str.strip()
    df = _build_feature_frame(raw_df)
    print(
        f"样本数: {len(df)}, 正样本: {(df[LABEL] == 1).sum()}, "
        f"负样本: {(df[LABEL] == 0).sum()}, source_key: {df['source_key'].nunique()}"
    )

    train_idx, test_idx = _group_split(df)
    train_df = df.iloc[train_idx].copy()
    test_df = df.iloc[test_idx].copy()
    x_train = train_df[FEATURES]
    y_train = train_df[LABEL]
    x_test = test_df[FEATURES]
    y_test = test_df[LABEL]
    print(f"训练集: {len(train_df)}, 测试集: {len(test_df)}")

    train_data = lgb.Dataset(x_train, label=y_train)
    test_data = lgb.Dataset(x_test, label=y_test, reference=train_data)
    params = {
        "objective": "binary",
        "metric": "auc",
        "boosting_type": "gbdt",
        "num_leaves": 63,
        "learning_rate": 0.04,
        "feature_fraction": 0.9,
        "bagging_fraction": 0.85,
        "bagging_freq": 5,
        "min_child_samples": 30,
        "verbose": -1,
        "num_threads": 4,
    }
    model = lgb.train(
        params,
        train_data,
        valid_sets=[test_data],
        num_boost_round=500,
        callbacks=[lgb.early_stopping(25), lgb.log_evaluation(50)],
    )

    predictions = model.predict(x_test)
    auc = roc_auc_score(y_test, predictions) if y_test.nunique() > 1 else 0.0
    acc = accuracy_score(y_test, (predictions > 0.5).astype(int))
    hit_metrics = _hit_at_k(test_df, predictions, ks=(1, 3, 5, 10))
    print("评估结果:")
    print(f"AUC: {auc:.4f}")
    print(f"Accuracy: {acc:.4f}")
    for key, value in hit_metrics.items():
        print(f"{key}: {value:.4f}")

    model_path = OUTPUT_DIR / "placement_model.txt"
    model.save_model(str(model_path))
    schema = {
        "model_type": "lightgbm_binary_v3_placement_model",
        "model_path": str(model_path),
        "training_data": str(data_path),
        "label": LABEL,
        "features": FEATURES,
        "contract": {
            "input": ["course_name", "teacher_no", "teacher_name", "class_name"],
            "output": ["day_of_week", "period_index", "classroom_name"],
        },
        "metrics": {
            "auc": round(float(auc), 4),
            "accuracy": round(float(acc), 4),
            **{key: round(float(value), 4) for key, value in hit_metrics.items()},
        },
        "training_samples": int(len(df)),
        "positive_samples": int((df[LABEL] == 1).sum()),
        "negative_samples": int((df[LABEL] == 0).sum()),
        "source_key_count": int(df["source_key"].nunique()),
        "notes": "V3 placement model trained only on theory/computer-lab courses and normal/lab rooms.",
    }
    schema_path = OUTPUT_DIR / "placement_model_schema.json"
    schema_path.write_text(json.dumps(schema, ensure_ascii=False, indent=2), encoding="utf-8")

    importance = pd.DataFrame({
        "feature": FEATURES,
        "gain": model.feature_importance(importance_type="gain"),
        "split": model.feature_importance(importance_type="split"),
    }).sort_values("gain", ascending=False)
    importance_path = OUTPUT_DIR / "placement_model_feature_importance.csv"
    importance.to_csv(importance_path, index=False)
    print(f"模型保存: {model_path}")
    print(f"Schema: {schema_path}")
    print(f"Feature importance: {importance_path}")
    return model_path


def _build_feature_frame(raw_df: pd.DataFrame) -> pd.DataFrame:
    df = raw_df.copy()
    for col in [
        "course_name", "course_code", "teacher_no", "teacher_name", "class_name",
        "class_major", "class_department", "course_type", "required_room_type",
        "classroom_name", "classroom_type",
    ]:
        df[f"{col}_code"] = df[col].map(_stable_code).astype(float)
    numeric_cols = [
        "label", "class_grade", "class_no", "student_count", "total_hours",
        "day_of_week", "period_index", "classroom_capacity", "capacity_margin",
        "capacity_ratio", "is_room_type_match",
    ]
    for col in numeric_cols:
        df[col] = pd.to_numeric(df[col], errors="coerce").fillna(0)
    df[LABEL] = df[LABEL].astype(int)
    return df


def _group_split(df: pd.DataFrame):
    splitter = GroupShuffleSplit(n_splits=1, test_size=0.2, random_state=42)
    groups = df["source_key"].astype(str)
    return next(splitter.split(df, df[LABEL], groups=groups))


def _hit_at_k(test_df: pd.DataFrame, predictions, *, ks: tuple[int, ...]) -> dict[str, float]:
    scored = test_df[[
        "source_key",
        "label",
        "classroom_name",
        "day_of_week",
        "period_index",
    ]].copy()
    scored["prediction"] = predictions
    hits = {k: 0 for k in ks}
    total = 0
    for _key, group in scored.groupby("source_key", dropna=False):
        positives = {
            (str(row.classroom_name), int(row.day_of_week), int(row.period_index))
            for row in group[group["label"] == 1].itertuples()
        }
        if not positives:
            continue
        total += 1
        ranked = group.sort_values("prediction", ascending=False)
        for k in ks:
            top = {
                (str(row.classroom_name), int(row.day_of_week), int(row.period_index))
                for row in ranked.head(k).itertuples()
            }
            if positives & top:
                hits[k] += 1
    denominator = max(1, total)
    return {f"hit@{k}": hits[k] / denominator for k in ks}


def _stable_code(value, modulo: int = 10007) -> int:
    text = str(value or "").strip().lower().replace(" ", "")
    if not text:
        return 0
    total = 0
    for char in text:
        total = (total * 131 + ord(char)) % modulo
    return total + 1


if __name__ == "__main__":
    train()
